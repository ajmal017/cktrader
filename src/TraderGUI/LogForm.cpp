...
void LogForm::adjustTableWidget(QTableWidget* tableWidget)
{
	...
	tableWidget->horizontalHeader()->setStyleSheet("QHeaderView::section{background-color: \
													qlineargradient(x1:0, y1:0, x2:0, y2:1,\
													stop:0 #FFFFFF, stop: 0.5 #F5F5F5,stop: \
													0.6 #E8E8E8, stop:1 #FFFFFF);color: black;\
													padding-left:4px; border: 0px; border-right:\
													1px solid GRAY;border-bottom: 1px solid GRAY;}"); //设置表头背景色=
...
void LogForm::onLog(Datablk& log)
{
	...
	emit updateEvent(log_data);
}

void LogForm::updateContent(LogData log)
{
	ui->logTableWidget->insertRow(0);

	QString content = codec->toUnicode(log.logContent.c_str());
	QString gate = codec->toUnicode(log.gateWayName.c_str());
	ui->logTableWidget->setItem(0, 0, new QTableWidgetItem(content));
	ui->logTableWidget->setItem(0, 1, new QTableWidgetItem(gate));

	ui->logTableWidget->horizontalHeader()->resizeSections(QHeaderView::ResizeToContents);
}